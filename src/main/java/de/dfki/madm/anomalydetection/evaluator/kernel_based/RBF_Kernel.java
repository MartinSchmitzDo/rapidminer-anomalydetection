...
public class RBF_Kernel {
	 ...
	 * @param y
	 * @param alpha
	 * @return a double array 'k' of size two: k[0] is the value of the kernel function, k[1] is the differential of the
	 * kernel function relative to alpha
	 */
	...
	/**
	 * Perform gradient ascent to maximize J= s^2 / (k_avg +eps) where s^2= sum_i_0_{l-1}(sum_j_{i+1}_{l-1}((k(i,j)-k_avg)^2))/(number-1)
	 * k_avg = sum_i_0_{l-1}(sum_j_{i+1}_{l-1}(k(i,j))/ number dJ/d_alpha = sum_i_0_{l-1}(sum_j_{i+1}_{l-1}( 2 * (k(i,j)
	 * - k_avg) * (k(i,j)' - k_avg') * (k_avg + eps) - k_avg' * (k(i,j) - k_avg) ^ 2 )/ (number-1) * (k_avg + eps) ^ 2
	 *
